...
function shinobiGarantirIdNotaEmAcao(topico){
  if(!topico||typeof topico!=="object")return "";
  if(!String(topico.id||"").trim())topico.id=shinobiNovoIdNota();
  return String(topico.id);
}
function salvarTopicosNotas(contexto={}){
  garantirTopicosNotas();
  const detalhe={
    confirmada:true,origem:"notas",campo:"notasTopicos",motivo:"alteracao-confirmada",
    collection:"notas",itemId:String(contexto.itemId||""),collectionAction:String(contexto.collectionAction||"upsert")
  };
  if(typeof persistirEstadoLocal==="function"){persistirEstadoLocal(detalhe);return;}
  if(typeof persistirSemRender==="function")persistirSemRender(detalhe);
}
...
async function editarTituloTopicoNota(i, ev){
  ...
  const itemId=shinobiGarantirIdNotaEmAcao(topico);

  salvarTopicosNotas({itemId,collectionAction:"upsert"});
  renderizarTopicosNotas();
}
