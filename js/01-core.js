...
function shinobiHashNotaLegada(texto){
  let h=2166136261;
...
function shinobiIdNotaLegada(item,indice){
  const personagem=String(estado?.__online?.characterId||estado?.characterId||fichaAtual||"Principal");
  return "nota_legacy_"+shinobiHashNotaLegada([personagem,indice,String(item?.titulo||""),String(item?.texto||"")].join("|"));
}
function garantirTopicosNotas(){
  let alterou=false;
  if(!Array.isArray(estado.notasTopicos)){
    estado.notasTopicos=[];
    alterou=true;
  ...
  estado.notasTopicos.forEach((item,indice)=>{
    if(!item||typeof item!=="object")return;
    if(!String(item.id||"").trim()){
      item.id=shinobiIdNotaLegada(item,indice);
      alterou=true;
    }
  });
  if(alterou&&typeof persistirEstadoLocal==="function"){
    persistirEstadoLocal({emitir:false,confirmada:true,origem:"migracao-notas",motivo:"ids-notas"});
  }
}
