  ...
  try{
    ...
    try{
      if(window.EkoRealtimeFields?.camposAlterados){
        ...
        const chaves=new Set([...Object.keys(estadoAnterior||{}),...Object.keys(estadoPersistido||{})]);
        camposAlterados=[...chaves].filter(chave=>JSON.stringify(estadoAnterior?.[chave])!==JSON.stringify(estadoPersistido?.[chave])).sort();
      }
    }catch(_erroDiff){}
    ...
    if(contexto.emitir!==false){
      try{
        window.dispatchEvent(new CustomEvent("shinobi:ficha-persistida",{detail:{
          ...
          savedAt:Date.now(),
          confirmada:contexto.confirmada===true,
