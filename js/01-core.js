  ...
  return new Promise(resolve=>{
    ...
    overlay.innerHTML = `
      <div class="modalShinobiBox" role="dialog" aria-modal="true">
        <h3 class="modalShinobiTitulo">${escaparHtmlShinobi(titulo || 'Confirmação')}</h3>
        <p class="modalShinobiTexto">${escaparHtmlShinobi(texto || '')}</p>
...
function normalizarImagemJutsuSegura(valor){
  ...
  if(!/^(?:blob:|data:image\/(?:png|jpe?g|webp|gif);base64,)/i.test(imagem)) return "";
  return imagem.replace(/[\r\n'"()\\]/g,caractere=>encodeURIComponent(caractere));
}
...
const APP_VERSION = window.APP_VERSION || "2.5.8.62";
window.APP_VERSION = APP_VERSION;

/* Ficha Ninja RPG — otimização v2 */

/* ===== Confirmação transacional de campos persistentes — v2.5.8.8 ===== */
const shinobiCamposConfirmando = new WeakSet();

function shinobiValorCampo(campo){
  return campo?.type==="checkbox" ? Boolean(campo.checked) : String(campo?.value ?? "");
}
...
function shinobiDesserializarValorCampo(valor,padrao=""){
  ...
  try{return JSON.parse(valor);}catch(_erro){return valor;}
}
function shinobiRotuloCampo(campo){
  ...
  const chave=String(campo?.dataset?.save||"");
  if(mapa[chave])return mapa[chave];
  if(campo?.id){
    const label=document.querySelector(`label[for="${String(campo.id).replace(/"/g,'\\"')}"]`);
    const textoLabel=String(label?.textContent||"").trim().replace(/\s+/g," ");
    if(textoLabel)return textoLabel.slice(0,70);
  }
  const nome=String(campo?.getAttribute?.("aria-label")||campo?.name||campo?.placeholder||chave||"Campo").trim();
  return nome||"Campo";
}
...
function shinobiAtualizarVisualSemSalvar(){
  try{atualizarPlacar();}catch(_erro){}
  try{atualizarPerfil();}catch(_erro){}
  try{window.atualizarDefesasTotaisBatalha?.();}catch(_erro){}
}
function shinobiRegistrarEdicaoCampo(evento){
  const campo=evento?.currentTarget;
  if(!campo?.dataset?.save)return;
  const atual=shinobiSerializarValorCampo(shinobiValorCampo(campo));
  const confirmado=campo.dataset.shinobiValorConfirmado;
  if(confirmado==null)campo.dataset.shinobiValorConfirmado=atual;
  campo.dataset.shinobiEdicaoPendente=atual===campo.dataset.shinobiValorConfirmado?"0":"1";
  shinobiAtualizarVisualSemSalvar();
}
async function shinobiConfirmarEdicaoCampo(evento){
  const campo=evento?.currentTarget;
  if(!campo?.dataset?.save||shinobiCamposConfirmando.has(campo))return;
  const novo=shinobiValorCampo(campo);
  const novoSerializado=shinobiSerializarValorCampo(novo);
  const antigo=shinobiDesserializarValorCampo(campo.dataset.shinobiValorConfirmado,
    campo.type==="checkbox"?Boolean(estado?.[campo.dataset.save]):String(estado?.[campo.dataset.save]??""));
  const antigoSerializado=shinobiSerializarValorCampo(antigo);
  ...
  try{
    const rotulo=shinobiRotuloCampo(campo);
    const ok=await modalShinobi(
      "Confirmar alteração?",
      `${rotulo}: ${shinobiTextoValorCampo(antigo)} → ${shinobiTextoValorCampo(novo)}`
    );
    if(!ok){
      if(campo.type==="checkbox")campo.checked=Boolean(antigo);
      else campo.value=String(antigo??"");
      campo.dataset.shinobiEdicaoPendente="0";
      shinobiAtualizarVisualSemSalvar();
      return;
    }
...
const CHAVE_BASE="ficha_ninja_app_v2",CHAVE_LISTA="ficha_ninja_lista_v1",CHAVE_ATIVA="ficha_ninja_ativa_v1";function limparNomeFicha(nome){return String(nome||"Principal").trim().replace(/[^\w\-À-ÿ ]+/g,"").slice(0,32)||"Principal"}function shinobiEhNomeCopiaAutomatica(nome){
  const valor=String(nome||"").trim();
  if(!valor||valor==="Principal")return false;
  if(/(?:\s+nuvem(?:\s+\d+)?)+$/i.test(valor))return true;
  return /^nuvem\s+[a-z0-9]{6,}$/i.test(valor);
}
function shinobiDadosLocaisPorChave(chave){
  try{
    const bruto=localStorage.getItem(chave);
...
function shinobiIdentidadesFicha(data){
  const online=data&&data.__online&&typeof data.__online==="object"?data.__online:{};
  return new Set([
    online.characterId,online.realtimeId,online.sheetId,online.sourceSheetId,online.originSheetId
  ].map(v=>String(v||"").trim()).filter(Boolean));
}
function shinobiCompartilhaIdentidade(dataA,dataB){
  const a=shinobiIdentidadesFicha(dataA),b=shinobiIdentidadesFicha(dataB);
  for(const id of a)if(b.has(id))return true;
  return false;
}
function shinobiNormalizarComparacaoLegada(valor,caminho=""){
  if(Array.isArray(valor))return valor.map((item,i)=>shinobiNormalizarComparacaoLegada(item,`${caminho}[${i}]`));
...
function adicionarJutsu(){estado.jutsus=estado.jutsus||[],estado.jutsus.push({nome:"",rank:"",elemento:"katon",bonusAcerto:"",dano:"",bonusDano:"",custo:"",alcance:"",raio:"",duracao:"",acao:"",resistencia:"",alvo:"",descricao:"",imagem:""}),persistirListas("jutsus")}function adicionarArmado(){estado.armados=estado.armados||[],estado.armados.push({nome:"",tipo:"armado",bonusAcerto:"",dano:"",bonusDano:"",obs:"",itemInventario:"",quantidadeUso:"1"}),persistirListas("armados")}async function removerJutsu(i){const j=(estado.jutsus||[])[i];if(!j)return;const nome=String(j.nome||"Jutsu");const ok=typeof modalShinobi==="function"?await modalShinobi("Remover jutsu?",`Remover “${nome}” da ficha?`):confirm(`Remover "${nome}" da ficha?`);if(!ok)return;estado.jutsus.splice(i,1),estado.jutsusAbertos={},persistirListas("jutsus")}async function removerArmado(i){const a=(estado.armados||[])[i];if(!a)return;const nome=String(a.nome||"Ataque");const ok=typeof modalShinobi==="function"?await modalShinobi("Remover ataque?",`Remover “${nome}” da ficha?`):confirm(`Remover "${nome}" da ficha?`);if(!ok)return;estado.armados.splice(i,1),estado.ataquesAbertos={},persistirListas("armados")}function persistirSemRender(contexto={}){persistirEstadoLocal(contexto)}function persistirListas(campo){persistirEstadoLocal({confirmada:true,origem:"colecao",campo:String(campo||""),motivo:"alteracao-confirmada"}),renderizarJutsus(),renderizarArmados()}function dadosElementoJutsu(elemento){const mapa={katon:{nome:"KATON",icone:"🔥",classe:"jutsu-katon"},raiton:{nome:"RAITON",icone:"⚡",classe:"jutsu-raiton"},fuuton:{nome:"FUUTON",icone:"🌪️",classe:"jutsu-fuuton"},suiton:{nome:"SUITON",icone:"💧",classe:"jutsu-suiton"},doton:{nome:"DOTON",icone:"🪨",classe:"jutsu-doton"},yin:{nome:"YINTON",icone:"🌑",classe:"jutsu-yin"},yang:{nome:"YOUTON",icone:"☀️",classe:"jutsu-yang"},neutro:{nome:"NEUTRO",icone:"✨",classe:"jutsu-neutro"}};return mapa[elemento]||mapa.neutro}function limparNumeroDano(valor){const texto=String(valor||"").trim().replace(",",".");if(!texto)return 0;const match=texto.match(/[-+]?\d+(\.\d+)?/);return match?Number(match[0]):0}function formatarDanoTotal(dano,bonus){const danoTexto=String(dano||"").trim(),bonusTexto=String(bonus||"").trim(),bonusNum=limparNumeroDano(bonusTexto);if(!danoTexto&&!bonusTexto)return"—";const danoEhNumero=/^[-+]?\d+([.,]\d+)?$/.test(danoTexto),bonusEhNumero=/^[-+]?\d+([.,]\d+)?$/.test(bonusTexto);return danoEhNumero&&bonusEhNumero?String(limparNumeroDano(danoTexto)+bonusNum):danoTexto&&bonusNum?danoTexto+" + "+bonusNum:danoTexto||(bonusTexto||"—")}function valorJutsu(j,campo,padrao="—"){return(j&&void 0!==j[campo]?String(j[campo]).trim():"")||padrao}function editarCampoJutsuPrompt(i,campo,rotulo,padrao=""){const j=(estado.jutsus||[])[i];if(!j)return;const atual=j[campo]||"",novo=prompt(rotulo,atual);null!==novo&&(j[campo]=novo.trim(),persistirSemRender({confirmada:true,origem:"jutsus",campo:"jutsus",motivo:"alteracao-confirmada"}),renderizarJutsus())}function escolherElementoJutsuPrompt(i){const j=(estado.jutsus||[])[i];if(!j)return;const atual=j.elemento||"katon",escolha=prompt(`Escolha o elemento:\n1 - Katon\n2 - Raiton\n3 - Fuuton\n4 - Suiton\n5 - Doton\n6 - Yinton\n7 - Youton\n8 - Neutro\n\nAtual: ${atual}`,"");if(null===escolha)return;const novo={1:"katon",2:"raiton",3:"fuuton",4:"suiton",5:"doton",6:"yin",7:"yang",8:"neutro"}[String(escolha).trim()]||String(escolha).trim().toLowerCase();["katon","raiton","fuuton","suiton","doton","yin","yang","neutro"].includes(novo)?(j.elemento=novo,persistirSemRender({confirmada:true,origem:"jutsus",campo:"jutsus",motivo:"alteracao-confirmada"}),renderizarJutsus()):alert("Elemento inválido.")}function jutsuAberto(i){return estado.jutsusAbertos=estado.jutsusAbertos||{},!!estado.jutsusAbertos[i]}function alternarJutsuAberto(i){estado.jutsusAbertos=estado.jutsusAbertos||{},estado.jutsusAbertos[i]=!estado.jutsusAbertos[i],persistirEstadoLocal({emitir:false,confirmada:true,origem:"ui",motivo:"jutsu-aberto"}),renderizarJutsus()}function renderizarJutsus(){const box=document.getElementById("listaJutsus");if(!box)return;const html=[];(estado.jutsus||[]).forEach((j,i)=>{const aberto=jutsuAberto(i),dados=dadosElementoJutsu(j.elemento||"katon"),nome=escaparHtmlShinobi(valorJutsu(j,"nome","Novo jutsu")),custo=escaparHtmlShinobi(valorJutsu(j,"custo","0")),rank=escaparHtmlShinobi(valorJutsu(j,"rank","Rank")),dano=escaparHtmlShinobi(valorJutsu(j,"dano","—")),totalDano=escaparHtmlShinobi(formatarDanoTotal(j.dano,j.bonusDano)),alcance=escaparHtmlShinobi(valorJutsu(j,"alcance","—")),raio=escaparHtmlShinobi(valorJutsu(j,"raio","—")),duracao=escaparHtmlShinobi(valorJutsu(j,"duracao","—")),bonusAcerto=escaparHtmlShinobi(valorJutsu(j,"bonusAcerto","—")),bonusDano=escaparHtmlShinobi(valorJutsu(j,"bonusDano","—")),acao=escaparHtmlShinobi(valorJutsu(j,"acao","—")),resistencia=escaparHtmlShinobi(valorJutsu(j,"resistencia","—")),alvo=escaparHtmlShinobi(valorJutsu(j,"alvo","—")),descricao=escaparHtmlShinobi(valorJutsu(j,"descricao","Toque para editar efeitos")),imagem=normalizarImagemJutsuSegura(j.imagem),estiloImagem=imagem?`style="background-image:url('${imagem}')"`:"";html.push(`\n      <div class="jutsuCard jutsuCardCompacto jutsuListaCard ${aberto?"jutsuAberto":""} ${imagem?"comImagem":"semImagem"} ${dados.classe}">\n        <div class="jutsuCartaFundo" ${estiloImagem}></div>\n        <button class="jutsuLinhaResumo" onclick="alternarJutsuAberto(${i})">\n          <span class="jutsuLinhaIcone">${dados.icone}</span>\n\n          <span class="jutsuLinhaTexto">\n            <strong>${nome}</strong>\n            <small>${dados.nome} • ${rank} • ${custo} CH</small>\n          </span>\n\n          <span class="jutsuLinhaSeta">${aberto?"▲":"▼"}</span>\n        </button>\n\n        <div class="jutsuDetalhesCompactos">\n          <div class="jutsuTopo jutsuTopoEditavel">\n            <button class="jutsuIcone jutsuEditavel" onclick="escolherElementoJutsuPrompt(${i})" title="Editar elemento">${dados.icone}</button>\n\n            <div class="jutsuTitulo jutsuNomeEditavel" onclick="editarCampoJutsuPrompt(${i},'nome','Nome do jutsu')">\n              <h3>${nome}</h3>\n              <span class="jutsuElementoLabel">${dados.nome}</span>\n            </div>\n\n            <button class="jutsuRankPill jutsuEditavel" onclick="editarCampoJutsuPrompt(${i},'rank','Rank do jutsu')">${rank}</button>\n            <button class="jutsuCustoPill jutsuEditavel" onclick="editarCampoJutsuPrompt(${i},'custo','Custo de Chakra')">${custo} CH</button>\n          </div>\n\n          <div class="jutsuCartaImagemAcoes">\n            <button type="button" onclick="abrirUploadImagemJutsu(${i})">🖼 Fundo da carta</button>\n            <button type="button" onclick="removerImagemJutsu(${i})">Remover fundo</button>\n          </div>\n\n          <div class="jutsuResumo jutsuResumoEditavel">\n            <button onclick="editarCampoJutsuPrompt(${i},'dano','Dano')"><b>Dano</b>${dano}</button>\n            <button class="danoTotalBox" onclick="editarCampoJutsuPrompt(${i},'bonusDano','Bônus de dano')"><b>Dano total</b>${totalDano}</button>\n            <button onclick="editarCampoJutsuPrompt(${i},'alcance','Alcance')"><b>Alcance</b>${alcance}</button>\n            <button onclick="editarCampoJutsuPrompt(${i},'raio','Raio / Área')"><b>Raio</b>${raio}</button>\n            <button onclick="editarCampoJutsuPrompt(${i},'duracao','Duração')"><b>Duração</b>${duracao}</button>\n            <button onclick="editarCampoJutsuPrompt(${i},'bonusAcerto','Bônus de acerto')"><b>Acerto</b>${bonusAcerto}</button>\n            <button onclick="editarCampoJutsuPrompt(${i},'bonusDano','Bônus de dano')"><b>Bônus dano</b>${bonusDano}</button>\n            <button onclick="editarCampoJutsuPrompt(${i},'acao','Tipo de ação')"><b>Ação</b>${acao}</button>\n            <button onclick="editarCampoJutsuPrompt(${i},'resistencia','Teste / Resistência')"><b>Teste</b>${resistencia}</button>\n            <button onclick="editarCampoJutsuPrompt(${i},'alvo','Alvo')"><b>Alvo</b>${alvo}</button>\n            <button class="jutsuResumoFull" onclick="editarCampoJutsuPrompt(${i},'descricao','Outros bônus / efeitos')"><b>Efeitos</b>${descricao}</button>\n          </div>\n\n          <div class="jutsuAcoes jutsuAcoesCompactas">\n            <button class="btn btnUsarJutsu" onclick="usarJutsu(${i})">USAR JUTSU</button>\n            <button class="btn perigo btnRemoverJutsu" onclick="removerJutsu(${i})">Remover</button>\n          </div>\n        </div>\n      </div>\n    `)}),box.innerHTML=html.join("")}async function usarJutsu(i){
  ...
  if(typeof window.aplicarEfeitosJutsuBatalha==="function"){
    await window.aplicarEfeitosJutsuBatalha(j,i);
  }
  atualizarHUD();
  if(typeof window.atualizarDefesasTotaisBatalha==="function")window.atualizarDefesasTotaisBatalha();
}function escolherTipoAtaquePrompt(i){const a=(estado.armados||[])[i];if(!a)return;const atual=a.tipo||"armado",escolha=prompt(`Tipo de ataque:\n1 - Armado\n2 - Desarmado\n\nAtual: ${atual}`,"");if(null===escolha)return;const valor=String(escolha).trim().toLowerCase(),novo="2"===valor||"desarmado"===valor?"desarmado":"armado";a.tipo=novo,persistirSemRender({confirmada:true,origem:"armados",campo:"armados",motivo:"alteracao-confirmada"}),renderizarArmados()}function iconeTipoAtaque(tipo){return"desarmado"===tipo?"👊🏼":"⚔️"}function labelTipoAtaque(tipo){return"desarmado"===tipo?"Ataque desarmado":"Ataque armado"}function referenciaIconeAtaque(a){if(!a)return"";const item=String(a.itemInventario||"").trim();return item||String(a.nome||"").trim()}function srcIconeInventarioCompartilhado(nome){if(typeof window.obterImagemInventarioPorNome!=="function")return"";return String(window.obterImagemInventarioPorNome(nome)||"")}function visualIconeAtaque(a,classeImagem=""){const tipo=a?.tipo||"armado";if("desarmado"===tipo)return'<span class="ataqueIconeFallback">👊🏼</span>';const src=srcIconeInventarioCompartilhado(referenciaIconeAtaque(a));return src?`<img class="ataqueArmaImagem ${classeImagem}" src="${src}" alt="" loading="lazy" decoding="async" draggable="false">`:'<span class="ataqueIconeFallback">⚔️</span>'}function visualIconeItemAtaque(a){const referencia=String(a?.itemInventario||"").trim();if(!referencia)return'<span class="ataqueItemUsadoFallback">🎒</span>';const src=srcIconeInventarioCompartilhado(referencia);return src?`<img class="ataqueItemUsadoImagem" src="${src}" alt="" loading="lazy" decoding="async" draggable="false">`:`<span class="ataqueItemUsadoFallback">${iconeInventario(referencia)}</span>`}function editarCampoArmadoPrompt(i,campo,rotulo){const a=(estado.armados||[])[i];if(!a)return;const atual=a[campo]||"",novo=prompt(rotulo,atual);null!==novo&&(a[campo]=novo.trim(),"nome"!==campo||a.itemInventario&&String(a.itemInventario).trim()||(a.itemInventario=novo.trim(),a.quantidadeUso||(a.quantidadeUso="1")),persistirSemRender({confirmada:true,origem:"armados",campo:"armados",motivo:"alteracao-confirmada"}),renderizarArmados())}function valorArmado(a,campo,padrao="—"){return(a&&void 0!==a[campo]?String(a[campo]).trim():"")||padrao}function ataqueAberto(i){return estado.ataquesAbertos=estado.ataquesAbertos||{},!!estado.ataquesAbertos[i]}function alternarAtaqueAberto(i){estado.ataquesAbertos=estado.ataquesAbertos||{},estado.ataquesAbertos[i]=!estado.ataquesAbertos[i],persistirEstadoLocal({emitir:false,confirmada:true,origem:"ui",motivo:"ataque-aberto"}),renderizarArmados()}function renderizarArmados(){const box=document.getElementById("listaArmados");if(!box)return;const html=[];(estado.armados||[]).forEach((a,i)=>{const aberto=ataqueAberto(i),tipo=a.tipo||"armado",classeTipo="desarmado"===tipo?"ataque-desarmado":"ataque-armado",nome=escaparHtmlShinobi(valorArmado(a,"nome","Novo ataque")),acerto=escaparHtmlShinobi(valorArmado(a,"bonusAcerto",valorArmado(a,"bonus","—"))),dano=escaparHtmlShinobi(valorArmado(a,"dano","—")),bonusDano=escaparHtmlShinobi(valorArmado(a,"bonusDano","—")),totalDano=escaparHtmlShinobi(formatarDanoTotal(a.dano,a.bonusDano)),obs=escaparHtmlShinobi(valorArmado(a,"obs","Toque para editar observações")),itemInventario=escaparHtmlShinobi(valorArmado(a,"itemInventario","Nenhum")),quantidadeUso=escaparHtmlShinobi(valorArmado(a,"quantidadeUso","1")),iconeResumo=visualIconeAtaque(a,"ataqueArmaImagemResumo"),iconeDetalhe=visualIconeAtaque(a,"ataqueArmaImagemDetalhe"),iconeItem=visualIconeItemAtaque(a),resumoTipo=escaparHtmlShinobi(labelTipoAtaque(tipo));html.push(`
      <div class="armadoCardCompacto ataqueListaCard ${classeTipo} ${aberto?"ataqueAberto":""}">
        <button class="ataqueLinhaResumo" onclick="alternarAtaqueAberto(${i})">
          <span class="ataqueLinhaIcone" onclick="event.stopPropagation(); escolherTipoAtaquePrompt(${i})">${iconeResumo}</span>

          <span class="ataqueLinhaTexto">
            <strong>${nome}</strong>
            <small>${resumoTipo}</small>
          </span>

          <span class="ataqueLinhaDano">${totalDano}</span>
          <span class="ataqueLinhaSeta" aria-hidden="true">${aberto?"▲":"›"}</span>
        </button>

        <div class="ataqueDetalhesCompactos">
          <div class="armadoTopo">
            <button class="armadoIcone" onclick="escolherTipoAtaquePrompt(${i})" aria-label="Editar tipo do ataque">${iconeDetalhe}</button>

            <div class="armadoNome" onclick="editarCampoArmadoPrompt(${i},'nome','Nome do ataque')">
              <h3>${nome}</h3>
              <span onclick="event.stopPropagation(); escolherTipoAtaquePrompt(${i})">${resumoTipo}</span>
            </div>
          </div>
          ...
          <div class="armadoResumoEditavel">
            ...
            <button onclick="vincularItemAtaque(${i})"><b>Item usado</b><span class="ataqueItemUsado">${iconeItem}<span>${itemInventario}</span></span></button>
            <button onclick="editarQtdUsoAtaque(${i})"><b>Qtd. por uso</b>${quantidadeUso}</button>
          </div>
...
function shinobiNovoIdNota(){
  try{
    if(window.crypto?.randomUUID)return "nota_"+window.crypto.randomUUID().replace(/-/g,"");
  }catch(_e){}
  return "nota_"+Date.now().toString(36)+"_"+Math.random().toString(36).slice(2,12);
}
...
function salvarTopicosNotas(contexto={}){
  ...
  const detalhe={
    ...
    motivo:"alteracao-confirmada",
    collection:"notas",
    itemId:String(contexto.itemId||""),
    collectionAction:String(contexto.collectionAction||"upsert")
  };

  if(typeof persistirEstadoLocal === "function"){
    persistirEstadoLocal(detalhe);
    return;
  }

  if(typeof persistirSemRender === "function"){
    persistirSemRender(detalhe);
  }
}
...
async function adicionarTopicoNota(){
  ...
  salvarTopicosNotas({itemId:novoTopico.id,collectionAction:"upsert"});
...
async function editarTituloTopicoNota(i, ev){
  ...
  if(!String(topico.id||"").trim())topico.id=shinobiNovoIdNota();
  topico.titulo = dados.titulo;
  topico.texto = dados.texto;

  salvarTopicosNotas({itemId:topico.id,collectionAction:"upsert"});
  renderizarTopicosNotas();
}
...
async function removerTopicoNota(i, ev){
  ...
  if(!String(topico.id||"").trim())topico.id=shinobiNovoIdNota();
  const itemId=String(topico.id);
  estado.notasTopicos.splice(i, 1);
  salvarTopicosNotas({itemId,collectionAction:"delete"});
  renderizarTopicosNotas();
}

async function confirmarTextoTopicoNota(i, valor){
  garantirTopicosNotas();

  const topico=estado.notasTopicos[i];
  if(!topico) return;

  const antigo=String(topico.texto||"");
  const novo=String(valor||"");
  if(antigo===novo) return;

  const ok=await modalShinobi(
    "Confirmar alteração?",
    `Salvar as alterações nas notas de “${topico.titulo||"Novo tópico"}”?`
  );
  if(!ok){
    renderizarTopicosNotas();
    return;
        ...
        <div class="topicoNotaCard ${aberto}">
          ...
          <div class="topicoNotaConteudo">
            <textarea
              ...
              onchange="confirmarTextoTopicoNota(${i}, this.value)"
            >${texto}</textarea>
          </div>
        </div>
      `;
