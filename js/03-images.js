...
(function(){
  ...
  function escaparImportacaoHtml(valor){
    if(typeof escaparHtmlShinobi === "function") return escaparHtmlShinobi(valor);
    return String(valor == null ? "" : valor)
      .replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;")
      .replace(/"/g,"&quot;").replace(/'/g,"&#039;");
  }

  function abrirEscolhaImportacao(){
    return new Promise(resolve=>{
      const overlay = document.createElement("div");
      overlay.className = "modalShinobiOverlay ekoImportOverlay";
      overlay.innerHTML = `
        <div class="modalShinobiBox ekoImportBox" role="dialog" aria-modal="true" aria-labelledby="ekoImportTitulo">
          <h3 class="modalShinobiTitulo" id="ekoImportTitulo">Importar ficha</h3>
          <p class="modalShinobiTexto">Escolha de onde você quer trazer a ficha completa.</p>
          <div class="ekoImportSourceGrid">
            <button type="button" class="ekoImportSource" data-import-source="device">
              <span class="shinobiIcon icon-upload" aria-hidden="true"></span>
              <strong>Dispositivo</strong>
              <small>Escolher um arquivo JSON salvo neste aparelho.</small>
            </button>
            <button type="button" class="ekoImportSource destaque" data-import-source="cloud">
              <span class="shinobiIcon icon-cloud" aria-hidden="true"></span>
              <strong>Nuvem</strong>
              <small>Baixar uma ficha completa da sua Conta Google.</small>
            </button>
          </div>
          <div class="modalShinobiAcoes somenteOk ekoImportCancelRow">
            <button type="button" class="modalShinobiBtn cancelar" data-import-source="cancel">Cancelar</button>
          </div>
        </div>`;

      const fechar = valor=>{ overlay.remove(); resolve(valor); };
      overlay.addEventListener("click",evento=>{
        const botao = evento.target.closest("[data-import-source]");
        if(botao) fechar(botao.dataset.importSource || "cancel");
        else if(evento.target === overlay) fechar("cancel");
      });
  ...
  function formatarDataBackup(timestamp){
    ...
    if(!valor) return "Backup na nuvem";
    try{
  ...
  function escolherFichaDaNuvem(fichas){
    return new Promise(resolve=>{
      const lista = Array.isArray(fichas) ? fichas : [];
      const overlay = document.createElement("div");
      overlay.className = "modalShinobiOverlay ekoImportOverlay";
      const itens = lista.map((ficha,indice)=>{
        const nome = escaparImportacaoHtml(ficha.characterName || ficha.name || "Ficha");
        const nomeBackup = ficha.name && ficha.name !== ficha.characterName
          ? `<small>${escaparImportacaoHtml(ficha.name)}</small>` : "";
        const selo = ficha.linked ? '<span class="ekoImportCloudBadge">Neste aparelho</span>' : "";
        return `<button type="button" class="ekoImportCloudItem" data-cloud-index="${indice}">
          <span class="ekoImportCloudMain"><strong>${nome}</strong>${nomeBackup}</span>
          <span class="ekoImportCloudMeta">${selo}<small>${escaparImportacaoHtml(formatarDataBackup(ficha.updatedAt))}</small></span>
        </button>`;
      }).join("");

      overlay.innerHTML = `
        <div class="modalShinobiBox ekoImportBox ekoImportCloudBox" role="dialog" aria-modal="true" aria-labelledby="ekoCloudTitulo">
          <h3 class="modalShinobiTitulo" id="ekoCloudTitulo">Fichas na nuvem</h3>
          <p class="modalShinobiTexto">Escolha qual personagem será baixado por completo.</p>
          <div class="ekoImportCloudList">${itens}</div>
          <div class="modalShinobiAcoes somenteOk ekoImportCancelRow">
            <button type="button" class="modalShinobiBtn cancelar" data-cloud-cancel>Cancelar</button>
          </div>
        </div>`;

      const fechar = valor=>{ overlay.remove(); resolve(valor); };
      overlay.addEventListener("click",evento=>{
        const item = evento.target.closest("[data-cloud-index]");
        if(item){
          const indice = Number(item.dataset.cloudIndex);
          fechar(Number.isInteger(indice) ? lista[indice] || null : null);
          return;
        }
        if(evento.target.closest("[data-cloud-cancel]") || evento.target === overlay) fechar(null);
      });
      document.body.appendChild(overlay);
    });
  }

  async function aguardarOnlineParaImportacao(){
    if(window.ShinobiOnline?.listarFichasNuvemAgora) return window.ShinobiOnline;
    return new Promise((resolve,reject)=>{
      let concluido = false;
      const finalizar = ()=>{
        if(concluido) return;
        concluido = true;
        clearTimeout(timer);
        if(window.ShinobiOnline?.listarFichasNuvemAgora) resolve(window.ShinobiOnline);
        else reject(new Error("O módulo online ainda não terminou de carregar."));
      };
      const timer = setTimeout(finalizar, 9000);
      window.addEventListener("shinobi:online-stack-ready", finalizar, {once:true});
      window.addEventListener("shinobi:online-stack-error", finalizar, {once:true});
    });
  }

  async function garantirOnlineInicializado(online){
    let status = online.snapshot?.() || {};
    if(status.iniciado && !status.carregando) return status;
    if(!status.carregando){
      await online.iniciar();
    ...
    return new Promise(resolve=>{
      ...
      const concluir = ()=>{
        ...
        terminou = true;
        clearTimeout(timer);
        limpezas.forEach(fn=>{ try{ fn?.(); }catch(_erro){} });
        resolve(online.snapshot?.() || {});
      };
      if(typeof online.on === "function"){
        ["pronto","erro","configuracao-pendente"].forEach(tipo=>limpezas.push(online.on(tipo,concluir)));
      }
      const timer = setTimeout(concluir, 9000);
    });
  }

  async function importarFichaDaNuvemInterativo(){
    try{
      const online = await aguardarOnlineParaImportacao();
      const status = await garantirOnlineInicializado(online);

      if(!status.user || status.user.anonymous){
        await avisar("Conta Google necessária", "Entre com sua Conta Google no modo online para importar uma ficha da nuvem.");
        return;
      }

      const fichas = await online.listarFichasNuvemAgora();
      if(!fichas.length){
        await avisar("Nenhuma ficha na nuvem", "Ainda não existe um backup completo disponível para esta Conta Google.");
        return;
      }

      const escolhida = await escolherFichaDaNuvem(fichas);
      if(!escolhida) return;

      const nome = escolhida.characterName || escolhida.name || "esta ficha";
      const textoConfirmacao = escolhida.linked
        ? `A versão local de “${nome}” será substituída pelo backup completo da nuvem. Continuar?`
        : `“${nome}” será baixada por completo para este aparelho. Continuar?`;
      const confirmou = typeof modalShinobi === "function"
        ? await modalShinobi(escolhida.linked ? "Restaurar da nuvem?" : "Importar da nuvem?", textoConfirmacao)
        : confirm(textoConfirmacao);
      if(!confirmou) return;

      await online.restaurarFichaDaNuvem(escolhida.id, {asCopy:false});
      alert("Ficha importada da nuvem com sucesso!");
    }catch(erro){
      console.error("Falha ao importar ficha da nuvem:", erro);
      const online = window.ShinobiOnline;
      const mensagem = online?.erroAmigavel?.(erro) || String(erro?.message || erro || "Não foi possível importar a ficha.");
      await avisar("Não foi possível importar da nuvem", mensagem);
    }
  }

  window.abrirImportarFicha = async function(){
    const origem = await abrirEscolhaImportacao();
    if(origem === "device"){
      const input = document.getElementById("importarFichaInput");
      if(input) input.click();
      return;
    }
    if(origem === "cloud") await importarFichaDaNuvemInterativo();
  };
