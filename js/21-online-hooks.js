...
(function(){
  ...
  async function enviarAlteracaoConfirmada(detalhe={}){
    ...
    if(campo==="notasTopicos"||campo==="inventarioItens"||campo==="jutsus"||campo==="armados"||campo==="efeitosBatalhaAtivos") return;
  ...
  async function enviarItemColecaoConfirmado(detalhe={}){
    ...
    if(!["notas","inventario","jutsus","armados","efeitosBatalha"].includes(collection)||!itemId) return;
  ...
  function instalarAutoSync(){
    ...
    window.addEventListener("shinobi:realtime-colecao-aplicada",evento=>{
      ...
      const campo=collection==="notas"?"notasTopicos":collection==="inventario"?"inventarioItens":collection==="jutsus"?"jutsus":collection==="armados"?"armados":"";
      if(!campo) return;
